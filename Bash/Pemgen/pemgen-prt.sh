...
sed -i -e 's/^ultraplay./New-WebBinding -Name "prod-pronet" -IPAddress "*" -Port 80 -HostHeader "ultraplay./' *omain*.txt
...
for D in ./*; do
	...
	if [ "$key" = "$cer" ]; then
		...
		echo ''
		echo ''
		if grep "PUBLIC" ../${PWD##*/}.pem > /dev/null
			then
				echo -e "\033[36m >>> Check ${PWD##*/}.pem file for PUBLIC block and remove it! <<<\033[m"
		fi
		if grep "REQUEST" ../${PWD##*/}.pem > /dev/null
			then
				echo -e "\033[36m >>> Check ${PWD##*/}.pem file for REQUEST block and remove it! <<<\033[m"
		fi
		echo ''
	else
